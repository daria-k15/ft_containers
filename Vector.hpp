...
namespace ft{
    ...
    class vector{
        public:
			...
			typedef typename ft::VectorIterator<T>::difference_type		diff_t;
        ...
        public:
			...
			vector (InputIterator first, InputIterator last, const allocator_type &alloc = allocator_type(), typename enable_if<!std::numeric_limits<InputIterator>::is_specialized>::type * = 0) : _alloc(alloc), _size(0), _capacity(20){
				if (first > last)
		...
		public:
			...
			allocator_type get_allocator() const{return std::allocator<T>(_alloc);}
	};
